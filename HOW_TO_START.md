...
3. ✅ **Auto-Seeds Database**: Checks if `arc_nomade.db` exists; if missing, automatically runs the database seeder (`database/seeds/seed_data.py`) to create sample trips, demo accounts, and expenses.
4. ✅ **Launches FastAPI Backend**: Activates `venv` and runs Uvicorn on `http://localhost:8000` with hot-reload in a separate console window.
5. ✅ **Launches Next.js Frontend**: Runs `npm run dev` on `http://localhost:3000` in a separate console window.
6. ✅ **Displays Links & Demo Accounts**: Shows direct links to Web UI, Swagger API docs, health check, and pre-seeded login credentials.
